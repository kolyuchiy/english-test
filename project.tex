...
\title{Контролирующая программа по английскому языку}
\author{Морев Николай, Забрудская Наталья}
...
что неизбежно приведет к излишним дополнительным трудностям и
проблемам. 
...
\section{Постановка задачи, подзадач}

Разработка программы для контроля знаний ученика в виде тестирования в 
среде Turbo Pascal 7.0.

\begin{enumerate}
\item Создание тестов по темам английского языка.
\item Создание пользовательского меню для выбора теста или получения справки 
и информации о программе.
\item Создание основы программы, проводящей тестирование, используя данные 
из файла теста, отформатированные специальным образом, понятным программе.
\item Создание подпрограммы для обработки и сохранения результатов
  теста.
\end{enumerate}

\section{Построение алгоритма}
\begin{enumerate}
\item Поиск имеющихся тестов в каталоге тестов и формирование массива для 
  пунктов меню.
\item Вывод меню и ожидание выбора пользователя:
  \begin{itemize}
  \item Пункт меню "Выход" осуществляет завершение программы.
  \item Пункты меню "О программе" и "Справка" выводят на экран 
    соответственно файлы "about.txt" и "help.txt", в которых находится 
    информация о программе и справка о пользовании программой.
  \item Пункты меню тестов запускают процедуру "test" для выбранного 
    пользователем файла теста, которая проводит тестирование.
  \end{itemize}
\item Процедура test последовательно задает тестируемому вопросы из файла 
...
\begin{itemize}
\item Всего в программе имеется четыре раздела по основным темам,
  изучаемым в школе:
  \begin{itemize}
  \item пассивный залог,
  \item согласование времен,
  \item наклонение,
  \item прямая и косвенная речь.
  \end{itemize}
\item Каждый из предложенных тестов состоит из вопросов имеющих 4 варианта 
  ответа, из которых только один правильный.
\item На каждый вопрос дается 60 секунд.
\item Результат теста сохраняется в файле "results".
\item Вы можете завершить тест нажатием  клавиши "Esc", при  этом текущие 
...
запуска программы:
\begin{itemize}
\item Компьютер IBM PC или совместимый с ним.
\item Видео карта VGA с памятью 256Kb и больше.
\item Операционная система MS DOS версии 3.0 и выше.
\end{itemize}
...
функции:
\begin{itemize}
\item lang.pas - модуль, позволяющий пользователю программы легко изменить 
  язык выводимых программой сообщений.
\item monobmp.pas - модуль для вывода на экран монохромных картинок в 
  формате BMP.
\item mycrypt.pas - модуль для зашифровки и расшифровки файлов теста.
\item project.pas - основная программа.
\end{itemize}
...
\bibitem{a} Качалова Н.В., Израилевич Б.Г. "Практическая грамматика
  английского языка". - М.: Вестник 1996.
\bibitem{b} Тесты 11 класс. Варианты и ответы централизованного
  тестирования - М.: Прометей 2000.
\bibitem{c} Тесты: Варианты и ответы централизованного тестирования. -
  М.: АСТ-ПРЕСС 2000.
\bibitem{d} Марченко А.И., Марченко Л.А. Программирование в среде Turbo
  Pascal 7.0 - К.: ВЕК+ 1999. 
\end{thebibliography}
